...
def load_sick_data():
	...
	train_labels = torch.tensor([1 if sick_data['label'][i]=='ENTAILMENT' else 0 if sick_data['label'][i]=='CONTRADICTION' else 2 
		for i in range(data_length) if sick_data['type'][i]=='TRAIN'])

	test_labels = torch.tensor([1 if sick_data['label'][i]=='ENTAILMENT' else 0 if sick_data['label'][i]=='CONTRADICTION' else 2 
		for i in range(data_length) if sick_data['type'][i]=='TEST'])

	valid_labels = torch.tensor([1 if sick_data['label'][i]=='ENTAILMENT' else 0 if sick_data['label'][i]=='CONTRADICTION' else 2 
		for i in range(data_length) if sick_data['type'][i]=='TRIAL'])
